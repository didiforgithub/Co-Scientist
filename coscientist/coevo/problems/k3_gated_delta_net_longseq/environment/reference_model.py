...
# --- workload distribution --- LONG-SEQUENCE variant.
# (B, T, H, HV, K, V). H=key/query heads, HV=value heads (GVA), K=V=128, chunk=64.
# Large T (B=1, H=8, HV=8, K=V=128): inter-chunk recurrence dominates cost, so the
# chunk-to-chunk state scan (not the intra-chunk matmul) is the optimization target.
WORKLOAD = [
    (1, 1024, 8, 8, 128, 128),
    (1, 2048, 8, 8, 128, 128),
    (1, 4096, 8, 8, 128, 128),
...
def make_inputs(shape, device, seed=0):
    ...
    g = -torch.rand(B, T, HV, device=device) * 0.1 - 0.01
