        ...
        K3 task: DPLR (longseq) — Diagonal-Plus-Low-Rank delta rule, larger T.
...
- `[1, 2048, 8, 128, 128]`
        ...
        `fla.generalized_delta_rule.dplr:chunk_dplr_delta_rule` anchor is `267.6`. Reward is:

        ```text
        clip(0.5 * log(score) / log(267.6), 0, 1)
        ```
