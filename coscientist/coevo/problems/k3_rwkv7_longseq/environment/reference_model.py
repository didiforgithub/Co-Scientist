...
# --- workload distribution --- LONG-SEQUENCE variant. (B, T, H, K, V). chunk=16.
# Large T (B=1, H=8, K=V=128): inter-chunk DPLR recurrence dominates cost. T
# capped at 2048 so the naive O(T) recurrent oracle stays tractable.
WORKLOAD = [
    (1, 1024, 8, 128, 128),
    (1, 2048, 8, 128, 128),
]
...
def _rand(shape, device, seed):
    ...
    # w: log-space per-channel decay. At long T the DPLR chunk kernel drifts from
    # the naive recurrence unless the state is contractive enough, so the decay is
    # made stronger than the base task (w in [-0.35, -0.05]) to keep them aligned.
    w = -torch.rand(B, T, H, K, device=device) * 0.3 - 0.05
